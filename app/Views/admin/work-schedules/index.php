...
ob_start();
?>
<section class="management-heading mb-4"><div><p class="dashboard-eyebrow mb-1">Master Data</p><h1 class="dashboard-title">Jadwal Kerja</h1><p class="dashboard-date mb-0">Kelola jam kerja dan tugaskan jadwal aktif kepada karyawan.</p></div><a class="btn btn-primary" href="<?= e(url('/admin/work-schedules/create')) ?>"><i class="bi bi-plus-lg me-1"></i>Tambah Jadwal</a></section>
<?php if ($success !== null): ?><div class="alert alert-success app-alert" role="alert"><i class="bi bi-check-circle me-2"></i><?= e($success) ?></div><?php endif; ?>
<?php if ($error !== null): ?><div class="alert alert-danger app-alert" role="alert"><i class="bi bi-exclamation-circle me-2"></i><?= e($error) ?></div><?php endif; ?>
...
<section class="dashboard-panel mb-4" aria-labelledby="schedules-title">
    ...
    <?php if ($schedules === []): ?>
        <div class="empty-state"><span class="empty-state-icon"><i class="bi bi-clock"></i></span><h3>Belum ada jadwal kerja.</h3><p>Tambahkan jadwal sebelum menugaskannya kepada karyawan.</p></div>
    <?php else: ?>
        <div class="table-responsive d-none d-md-block"><table class="table app-table align-middle mb-0"><thead><tr><th>Nama Jadwal</th><th>Jam Masuk</th><th>Jam Pulang</th><th>Toleransi</th><th>Status</th><th class="text-end">Aksi</th></tr></thead><tbody>
        <?php foreach ($schedules as $schedule): ?><tr><td class="fw-semibold"><?= e($schedule['name']) ?></td><td><?= e(substr((string) $schedule['start_time'], 0, 5)) ?></td><td><?= e(substr((string) $schedule['end_time'], 0, 5)) ?></td><td><?= e($schedule['late_tolerance_minutes']) ?> menit</td><td><span class="badge <?= (int) $schedule['is_active'] === 1 ? 'text-bg-success' : 'text-bg-secondary' ?>"><?= (int) $schedule['is_active'] === 1 ? 'Aktif' : 'Nonaktif' ?></span></td><td><div class="record-actions justify-content-end"><a class="btn btn-sm btn-outline-primary" href="<?= e(url('/admin/work-schedules/edit?id=' . $schedule['id'])) ?>">Edit</a><form method="post" action="<?= e(url('/admin/work-schedules/toggle')) ?>"><?= csrf_field() ?><input type="hidden" name="id" value="<?= e($schedule['id']) ?>"><button class="btn btn-sm <?= (int) $schedule['is_active'] === 1 ? 'btn-outline-danger' : 'btn-outline-success' ?>" type="submit"><?= (int) $schedule['is_active'] === 1 ? 'Nonaktifkan' : 'Aktifkan' ?></button></form></div></td></tr><?php endforeach; ?>
        </tbody></table></div>
        <div class="mobile-record-list d-md-none"><?php foreach ($schedules as $schedule): ?><article class="mobile-record"><div class="d-flex justify-content-between gap-3"><div><h3><?= e($schedule['name']) ?></h3><p><?= e(substr((string) $schedule['start_time'], 0, 5)) ?> - <?= e(substr((string) $schedule['end_time'], 0, 5)) ?></p></div><span class="badge align-self-start <?= (int) $schedule['is_active'] === 1 ? 'text-bg-success' : 'text-bg-secondary' ?>"><?= (int) $schedule['is_active'] === 1 ? 'Aktif' : 'Nonaktif' ?></span></div><p class="mb-3"><small>Toleransi terlambat</small><br><strong><?= e($schedule['late_tolerance_minutes']) ?> menit</strong></p><div class="record-actions"><a class="btn btn-sm btn-outline-primary" href="<?= e(url('/admin/work-schedules/edit?id=' . $schedule['id'])) ?>">Edit</a><form method="post" action="<?= e(url('/admin/work-schedules/toggle')) ?>"><?= csrf_field() ?><input type="hidden" name="id" value="<?= e($schedule['id']) ?>"><button class="btn btn-sm <?= (int) $schedule['is_active'] === 1 ? 'btn-outline-danger' : 'btn-outline-success' ?>" type="submit"><?= (int) $schedule['is_active'] === 1 ? 'Nonaktifkan' : 'Aktifkan' ?></button></form></div></article><?php endforeach; ?></div>
    <?php endif; ?>
</section>
...
<section class="dashboard-panel" aria-labelledby="assignment-title">
    ...
    <?php if ($employees === []): ?>
        <div class="empty-state"><span class="empty-state-icon"><i class="bi bi-people"></i></span><h3>Belum ada karyawan aktif.</h3><p>Penugasan jadwal akan tersedia setelah data karyawan aktif tersedia.</p></div>
    <?php else: ?>
