...
<div class="attendance-evidence-grid">
    <section class="dashboard-panel" aria-labelledby="check-in-evidence-title">
        ...
        <div class="attendance-evidence-body">
            <?php if ($attendance['has_check_in_photo']): ?><div class="attendance-selfie-frame"><img src="<?= e(url('/admin/attendances/photo?id=' . $attendance['id'] . '&type=check_in')) ?>" alt="Selfie absen masuk <?= e($attendance['name']) ?>"></div><?php else: ?><div class="attendance-photo-empty"><i class="bi bi-image" aria-hidden="true"></i><span>Selfie belum tersedia.</span></div><?php endif; ?>
            <dl class="attendance-evidence-data">
                <div><dt>Lokasi Kerja Check-in</dt><dd><?= e($attendance['work_location_name'] ?? 'Lokasi tidak tersedia') ?></dd></div>
    ...
    <section class="dashboard-panel" aria-labelledby="check-out-evidence-title">
        ...
        <div class="attendance-evidence-body">
            <?php if ($attendance['has_check_out_photo']): ?><div class="attendance-selfie-frame"><img src="<?= e(url('/admin/attendances/photo?id=' . $attendance['id'] . '&type=check_out')) ?>" alt="Selfie absen pulang <?= e($attendance['name']) ?>"></div><?php else: ?><div class="attendance-photo-empty"><i class="bi bi-image" aria-hidden="true"></i><span>Selfie belum tersedia.</span></div><?php endif; ?>
            <dl class="attendance-evidence-data">
            ...
            <p class="attendance-checkout-limitation mb-0"><i class="bi bi-info-circle" aria-hidden="true"></i><span>Nama lokasi check-out tidak tersimpan pada schema saat ini. Data yang tersedia adalah jarak validasi, akurasi, dan koordinat saat check-out.</span></p>
        </div>
    </section>
</div>
